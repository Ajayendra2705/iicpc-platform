...
type SubmissionServiceClient interface {
	...
	StreamBuildLogs(ctx context.Context, in *StreamBuildLogsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[BuildLogChunk], error)
}
...
func (c *submissionServiceClient) StreamBuildLogs(ctx context.Context, in *StreamBuildLogsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[BuildLogChunk], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &SubmissionService_ServiceDesc.Streams[0], SubmissionService_StreamBuildLogs_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[StreamBuildLogsRequest, BuildLogChunk]{ClientStream: stream}
...
type SubmissionService_StreamBuildLogsClient = grpc.ServerStreamingClient[BuildLogChunk]
...
type SubmissionServiceServer interface {
	...
	StreamBuildLogs(*StreamBuildLogsRequest, grpc.ServerStreamingServer[BuildLogChunk]) error
...
func (UnimplementedSubmissionServiceServer) StreamBuildLogs(*StreamBuildLogsRequest, grpc.ServerStreamingServer[BuildLogChunk]) error {
...
func _SubmissionService_StreamBuildLogs_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(StreamBuildLogsRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(SubmissionServiceServer).StreamBuildLogs(m, &grpc.GenericServerStream[StreamBuildLogsRequest, BuildLogChunk]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type SubmissionService_StreamBuildLogsServer = grpc.ServerStreamingServer[BuildLogChunk]
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _SubmissionService_StreamBuildLogs_Handler,
			ServerStreams: true,
		},
	},
