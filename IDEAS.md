...
- **Differential testing:** ✅ **DONE** — both matching engines are now driven against an independently-written brute-force price-time-priority oracle over 2 000 randomised streams each, asserting identical fills and book state after every op: the **validator** book in `services/validator/internal/replay/differential_test.go` and the contestant-facing **reference orderbook** in `samples/reference-orderbook/engine/differential_test.go`. Both matching their oracle guarantees the engine contestants build against and the engine that scores them agree — the platform's core fairness invariant — without coupling the two modules. (A live three-way diff of the bot's claimed fills is still possible as a future extension.)
